 ...
 */
static hccapx_t hccapx = {.signature = HCCAPX_SIGNATURE,
                          .version = 4,
                          .message_pair = 255,
 ...
 */
static unsigned save_eapol(eapol_packet_t *eapol_packet,
                           eapol_key_packet_t *eapol_key_packet) {
  unsigned eapol_len = 0;
  eapol_len = sizeof(eapol_packet_header_t) +
              ntohs(eapol_packet->header.packet_body_length);
  if (eapol_len > HCCAPX_MAX_EAPOL_SIZE) {
    ESP_LOGW(TAG, "EAPoL is too long (%u/%u)", eapol_len,
             HCCAPX_MAX_EAPOL_SIZE);
    return 1;
  }
  ...
  // Clear key MIC from EAPoL packet so hashcat can calulate MIC without
  // preprocessing. This is not documented in HCCAPX reference. But it's based
  // on 802.11i-2004 [8.5.2/h] and by analysing behaviour of cap2hccapx tool MIC
  // key on 77 bytes offset inside EAPoL-Key + 4 bytes EAPoL header.
 ...
 */
static void ap_message_m3(eapol_packet_t *eapol_packet,
                          eapol_key_packet_t *eapol_key_packet) {
  ESP_LOGD(TAG, "From AP M3");
  message_ap = 3;
  if (message_ap == 0) {
    // No AP message was processed yet. ANonce has to be copied into HCCAPX
    // buffer.
    memcpy(hccapx.nonce_ap, eapol_key_packet->key_nonce, 32);
  }
  if (eapol_source == 2) {
 ...
 * @param eapol_key_packet
 */
static void sta_message_m2(eapol_packet_t *eapol_packet,
                           eapol_key_packet_t *eapol_key_packet) {
 ...
 * @param eapol_key_packet
 */
static void sta_message_m4(eapol_packet_t *eapol_packet,
                           eapol_key_packet_t *eapol_key_packet) {
  ESP_LOGD(TAG, "From STA M4");
  if ((message_sta == 2) && (eapol_source != 0)) {
    // If message 2 was already fully processed, there is no need to process M4
    // again
...
/**
 * @detail This component is a state machine, so this function can be used
 * without knowing current state from outside. WPA handshake pseudo-diagram:
 * @code{.unparsed}
 * AP           STA
 * M1 ---------> |
 * | <--------- M2
...
void hccapx_serializer_add_frame(data_frame_t *frame) {
  ...
  // Determine direction of the frame by comparing BSSID (addr3) with source
  // address (addr2)
  if (memcmp(frame->mac_header.addr2, frame->mac_header.addr3, 6) == 0) {
    ...
    ESP_LOGE(TAG,
             "Unknown frame format. BSSID is not source nor destionation.");
  }
}