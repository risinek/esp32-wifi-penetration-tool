 ...
 *
 * It forwards captured frames into event pool and sorts them based on their
 * type
 * - Data
 * - Management
 * - Control
...
static void frame_handler(void *buf, wifi_promiscuous_pkt_type_t type) {
  ...
  ESP_ERROR_CHECK(esp_event_post(
      SNIFFER_EVENTS, event_id, frame,
      frame->rx_ctrl.sig_len + sizeof(wifi_promiscuous_pkt_t), portMAX_DELAY));
}

/**
 * @see
 * https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/network/esp_wifi.html#_CPPv425wifi_promiscuous_filter_t
 */
