...
static httpd_uri_t uri_root_get = {.uri = "/",
                                   .method = HTTP_GET,
...
static httpd_uri_t uri_reset_head = {.uri = "/reset",
                                     .method = HTTP_HEAD,
 ...
 * It calls wifi_controller ap_scanner and serialize their SSIDs into octet
 * response.
...
static httpd_uri_t uri_ap_list_get = {.uri = "/ap-list",
                                      .method = HTTP_GET,
 ...
 *
 * This endpoint receives attack configuration from client. It deserialize it
 * from octet stream to attack_request_t structure.
 * @param req
 * @return esp_err_t
 * @{
 */
...
static httpd_uri_t uri_run_attack_post = {.uri = "/run-attack",
                                          .method = HTTP_POST,
                                          .handler =
                                              uri_run_attack_post_handler,
                                          .user_ctx = NULL};
//@}
 ...
 *
 * This endpoint fetches current status from main component attack wrapper,
 * serialize it and sends it to client as octet stream.
 * @param req
 * @return esp_err_t
 * @{
 */
static esp_err_t uri_status_get_handler(httpd_req_t *req) {
  ...
  if (((attack_status->state == FINISHED) ||
    ...
    ESP_ERROR_CHECK(httpd_resp_send_chunk(req, attack_status->content,
                                          attack_status->content_size));
  }
  return httpd_resp_send_chunk(req, NULL, 0);
}

static httpd_uri_t uri_status_get = {.uri = "/status",
                                     .method = HTTP_GET,
 ...
 *
 * This endpoint forwards PCAP binary data from pcap_serializer via octet stream
 * to client.
 *
...
static httpd_uri_t uri_capture_pcap_get = {.uri = "/capture.pcap",
                                           .method = HTTP_GET,
                                           .handler =
                                               uri_capture_pcap_get_handler,
                                           .user_ctx = NULL};
//@}
 ...
 *
 * This endpoint forwards HCCAPX binary data from hccapx_serializer via octet
 * stream to client.
 *
 * @note Most browsers will start download process when this endpoint is called.
 * @param req
...
static httpd_uri_t uri_capture_hccapx_get = {.uri = "/capture.hccapx",
                                             .method = HTTP_GET,
                                             .handler =
                                                 uri_capture_hccapx_get_handler,
                                             .user_ctx = NULL};
//@}
