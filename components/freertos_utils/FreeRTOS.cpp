 ...
 */
void FreeRTOS::sleep(uint32_t ms) { ::vTaskDelay(ms / portTICK_PERIOD_MS); }  // sleep
 ...
 */
void FreeRTOS::deleteTask(TaskHandle_t pTask) { ::vTaskDelete(pTask); }  // deleteTask
...
std::string FreeRTOS::Semaphore::toString() {
  ...
  stringStream << "name: " << m_name << " (0x" << std::hex << std::setfill('0') << (uint32_t)m_semaphore
               << "), owner: " << m_owner;
 ...
 */
void FreeRTOS::Semaphore::setName(std::string name) { m_name = name; }  // setName
