...
static void data_frame_handler(void *args, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if(search_type == SEARCH_HANDSHAKE){
        ...
        ESP_ERROR_CHECK(esp_event_post(DATA_FRAME_EVENTS, DATA_FRAME_EVENT_EAPOLKEY_PACKET, eapol_packet, sizeof(eapol_packet_header_t) + eapol_packet->header.packet_body_length, portMAX_DELAY));
        return;
    }
