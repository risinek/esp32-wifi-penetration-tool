...
static pmkid_item_t *parse_pmkid_from_key_data(uint8_t *key_data, const uint16_t length) {
  ...
  do {
    ...
    ESP_LOGV(TAG, "EAPOL-Key -> Key-Data -> type=%x; length=%x; oui=%x; data_type=%x", key_data_field->type,
             key_data_field->length, key_data_field->oui, key_data_field->data_type);
