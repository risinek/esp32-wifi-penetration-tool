 ...
 *
 * If DATA_FRAME_EVENT_PMKID is received from event pool, this function stops
 * PMKID attack and serialize captured PMKIDs into status content.
 *
 * @param args not used
 ...
 */
static void pmkid_exit_condition_handler(void *args,
  ...
  char *content = attack_alloc_result_content(
      6 + 6 + 1 + strlen((char *)ap_record->ssid) + (pmkid_item_count * 16));
...
void attack_pmkid_stop() {
  ...
  ESP_ERROR_CHECK(esp_event_handler_unregister(
      ESP_EVENT_ANY_BASE, ESP_EVENT_ANY_ID, &pmkid_exit_condition_handler));
