 ...
 *
 * If EAPOL-Key frame is captured and DATA_FRAME_EVENT_EAPOLKEY_FRAME event is
 * received from event pool, this method appends the frame to status content and
 * serialize them into pcap and hccapx format.
 *
 * @param args not used
...
void attack_handshake_start(attack_config_t *attack_config) {
  ...
  ESP_ERROR_CHECK(esp_event_handler_register(FRAME_ANALYZER_EVENTS,
  ...
  default:
    ESP_LOGD(TAG,
             "Method unknown! Fallback to ATTACK_HANDSHAKE_METHOD_PASSIVE");
  }
}
