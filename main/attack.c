...
static attack_status_t attack_status = {
    .state = READY, .type = -1, .content_size = 0, .content = NULL};
...
void attack_append_status_content(uint8_t *buffer, unsigned size) {
  ...
  char *reallocated_content =
      realloc(attack_status.content, attack_status.content_size + size);
 ...
 *
 * This callback resets attack status by freeing previously allocated status
 * content and putting attack to READY state.
 *
 * @param args not used
 * @param event_base expects WEBSERVER_EVENTS
 ...
 */
void attack_init() {
  const esp_timer_create_args_t attack_timeout_args = {.callback =
                                                           &attack_timeout};
  ESP_ERROR_CHECK(
  ...
  ESP_ERROR_CHECK(esp_event_handler_register(WEBSERVER_EVENTS,
                                             ...
                                             &attack_request_handler, NULL));
  ESP_ERROR_CHECK(esp_event_handler_register(WEBSERVER_EVENTS,
                                             WEBSERVER_EVENT_ATTACK_RESET,
                                             &attack_reset_handler, NULL));
}