 ...
 *
 * This file provide interface to control attack wrapper like setting current
 * attack state, update attack status content, etc...
 */
 ...
 */
typedef enum {
  READY, ///< no attack is in progress and results from previous attack run are
         ///< available.
  RUNNING,  ///< attack is in progress, attack_status_t.content may not be
            ///< consistent.
  FINISHED, ///< last attack finsihed and results are available.
  TIMEOUT ///< last attack timed out. This option will be moved as sub category
          ///< of FINISHED state.
} attack_state_t;
 ...
 *
 * This function creates all necessary resources for attack wrapper. It has to
 * be called before any attack can be run.
 */
void attack_init();
